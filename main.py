...
SENSITIVE_KEYS = {"api_token", "nas_pass", "pc_pass"}
...
class PCControlPlugin(Star):
    ...
    def get_config(self) -> dict[str, Any]:
        ...
        for key, val in self._load_local_config().items():
            if key in CONFIG_KEYS and val not in (None, "", [], {}):
                cfg[key] = val
        for key in ("web_port", "nas_ssh_port", "pc_ssh_port", "shutdown_delay"):
            try:
                cfg[key] = int(cfg.get(key) or DEFAULTS[key])
            except Exception:
                cfg[key] = DEFAULTS[key]
        cfg["allow_unsafe_without_token"] = bool(cfg.get("allow_unsafe_without_token"))
        return cfg
    ...
    def wol(self) -> ActionResult:
        ...
        try:
            ...
            return ActionResult(ok, "📡 唤醒信号已发送" if ok else f"❌ 唤醒失败：{detail[:200]}")
        except Exception as exc:
    ...
    def shutdown_pc(self) -> ActionResult:
        ...
        try:
            ...
            return ActionResult(ok, f"🔌 关机指令已发送（{delay} 秒后执行）" if ok else f"❌ 关机失败：{detail[:200]}")
        except Exception as exc:
...
const defs=[['web_host','Web 监听地址','0.0.0.0'],['web_port','Web 端口','5800'],['api_token','API Token','',true],['allow_unsafe_without_token','未配置 Token 时允许控制(true/false)','false'],['nas_ip','NAS/跳板机 IP',''],['nas_user','NAS SSH 用户','root'],['nas_pass','NAS SSH 密码','',true],['nas_ssh_port','NAS SSH 端口','22'],['nas_target','NAS 测试 URL','http://...'],['pc_ip','PC IP',''],['pc_user','Windows SSH 用户',''],['pc_pass','Windows SSH 密码','',true],['pc_ssh_port','Windows SSH 端口','22'],['pc_mac','PC MAC','AA:BB:CC:DD:EE:FF'],['broadcast_ip','WOL 广播地址','192.168.31.255'],['game_process','目标进程名','StarRail.exe'],['shutdown_delay','关机延迟秒数','10']];
